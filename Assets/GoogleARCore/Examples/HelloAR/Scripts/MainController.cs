        ...
        public void Update()
        {
            ...
            if (Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit))
            {
                ...
                else
                {
					...
					var cubeObject = Instantiate(cubePrefab, hit.Pose.position + Vector3.up, hit.Pose.rotation);

					// Compensate for the hitPose rotation facing away from the raycast (i.e. camera).
					cubeObject.transform.Rotate(0, 0, 0, Space.Self);
					cubeObject.GetComponent<MeshRenderer>().material.color = Random.ColorHSV();
			...
			foreach (var t in Input.touches) {
				...
				if (Physics.Raycast(ray, out hitInfo)) {
					var go = Instantiate(cubePrefab, hitInfo.point + Vector3.up, Quaternion.identity);
